...
The system SHALL introduce a 2-second delay before executing Machine move to simulate human behavior.

#### Scenario: Delay before move broadcast
- **WHEN** Machine selects a move
- **THEN** the system SHALL wait 2000ms before applying move
- **AND** not display visible "thinking" message to client

#### Scenario: Delay does not block server
- **WHEN** thinking delay is active
- **THEN** the system SHALL use non-blocking setTimeout
